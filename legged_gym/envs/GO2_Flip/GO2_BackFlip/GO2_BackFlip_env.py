...
class Go2_BackFlip(BaseTask):
    ...
    def check_jump(self):
        ...
        # self.settled_after_init[torch.logical_or(has_jumped,self.was_in_flight)] = False #初始化完成      
    ...
    def check_termination(self):
        ...
        self.reset_buf[self.root_states[:,2]  <= 0.15] = True # check if the robot base is below 0.1 metres
    ...
    def reset_idx(self, env_ids):
        ...
        self.setted_frames[env_ids] = 250
    ...
    def _reward_before_setting(self):
        ...
        rew+=self.settled_after_init*torch.clip(100-self.setted_frames,min=0)/400
        # print(torch.sum(torch.abs(self.dof_pos-self.lie_joint_pos)/2,dim=1),self.settled_after_init[0])
        return rew
    ...
    def _reward_orientation(self): 
        #奖励身体姿态保持平衡
        rew=torch.exp(-torch.norm(torch.square(self.projected_gravity - self.target_gravity), dim=1)*5)*self.has_jumped
        return rew

    def _reward_angle_y(self):
        # print(quat_rotate(self.base_quat,torch.tensor([0., 1., 0.],device="cuda").repeat(self.num_envs,1)).shape)
        rew=torch.exp(-torch.norm(quat_rotate(self.base_quat,torch.tensor([0., 1., 0.],device="cuda").repeat(self.num_envs,1))-torch.tensor([0., 1., 0],device="cuda"),dim=1))*self.was_in_flight
        # print(rew.shape)
        return rew


    def _reward_ang_vel_y(self):
        rew = self.root_states[:, 11]*2-torch.abs(self.root_states[:, 10])-torch.abs(self.root_states[:, 12])*(self.was_in_flight)
        rew = self.root_states[:, 11]*2-torch.abs(self.root_states[:, 10])-torch.abs(self.root_states[:, 12])*(0.1*self.settled_after_init*~(self.was_in_flight))
        return rew
    
    def _reward_sys(self):

        rew=torch.abs(self.dof_pos[:,0]+self.dof_pos[:,3])+torch.abs(self.dof_pos[:,1]-self.dof_pos[:,4])+torch.abs(self.dof_pos[:,2]-self.dof_pos[:,5]) + \
               torch.abs(self.dof_pos[:,6]+self.dof_pos[:,9])+torch.abs(self.dof_pos[:,7]-self.dof_pos[:,10])+torch.abs(self.dof_pos[:,8]-self.dof_pos[:,11]) 
        rew= torch.exp(-rew*4)
        return rew
    def _reward_default_pose_air(self):
        #在空中的默认关节奖励
        angle_diff = torch.square(self.dof_pos - self.default_dof_pos).sum(dim=1)*(self.was_in_flight|self.has_jumped)
        return angle_diff
    ...
    def _reward_feet_contact_forces(self):
        ...
        return torch.sum((torch.norm(self.contact_forces[:, self.feet_indices, :], dim=-1) -  self.cfg.rewards.max_contact_force).clip(min=0.), dim=1)*self.has_jumped
