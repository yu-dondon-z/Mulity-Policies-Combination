...
from legged_gym.envs.GO2_Mulitpolicy.GO2_TrottoJump.GO2_TrottoJump_config2 import GO2_TrottoJump_Cfg,GO2_TrottoJump_PPO
...
class GO2_TrottoJump_Robot(BaseTask):
    ...
    #     contact_error = torch.sum(torch.abs(contact - target_contact), dim=1)
    #     jump_shape = torch.exp(-contact_error)

    #     cmd_x = torch.abs(self.commands[:, 0])
    #     vel_x = torch.abs(self.base_lin_vel[:, 0])
    #     vel_track = torch.exp(-torch.square(cmd_x - vel_x) / 0.1)  # 跟速度命令对齐
    #     active = (torch.norm(self.commands[:, :2], dim=1) > 0.2).float()

    #     return jump_shape * vel_track * active
    def _reward_jump(self):
        contact = (self.contact_forces[:, self.feet_indices, 2] > 5.).float()
        stance_mask = self._get_gait_phase().float()
        target_contact = stance_mask[:, 0].unsqueeze(1).repeat(1, 4)
        ...
        cmd_x = self.commands[:, 0]            # 不取 abs
        vel_x = self.base_lin_vel[:, 0]        # 不取 abs
        vel_track = torch.exp(-torch.square(cmd_x - vel_x) / 0.25)

        stuck = ((torch.abs(cmd_x) > 0.3) & (torch.abs(vel_x) < 0.12)).float()

        return jump_shape * vel_track - 0.5 * stuck
